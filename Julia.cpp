...
void Julia::creeLaMatrice(vector<unsigned char> &matrice, int longueur, int hauteur, double echelle, Complexe origine){
    ...
    for (y = 0; y <= hauteur - 1; y += 2){ // on parcourt les lignes puis les colonnes
		...
		thread a(calculeLigne, y, longueur, hauteur, echelle, origine,
                        borneDIteration, peindreEnBlanc, convergenceDuPoint, std::ref(matrice));
		thread b(calculeLigne, y+1, longueur, hauteur, echelle, origine,
                        borneDIteration, peindreEnBlanc, convergenceDuPoint, std::ref(matrice));
		a.join();
		b.join();
	...
	if (y != hauteur) { // s'il y a un nombre impair de lignes
		thread a(calculeLigne, y, longueur, hauteur, echelle, origine, borneDIteration, peindreEnBlanc, convergenceDuPoint, std::ref(matrice));
		a.join();
	}
}
